...
import { generateUniqueProductCode } from '../../../utils/productApi';
...
const RegisterDialog: React.FC<RegisterDialogProps> = ({ open, onClose, onSubmit }) => {
  const [form, setForm] = useState<Partial<ProductDTO>>({ code: generateUniqueProductCode() });
  useEffect(() => { if (open) setForm({ code: generateUniqueProductCode() }); }, [open]);
  if (!open) return null;
